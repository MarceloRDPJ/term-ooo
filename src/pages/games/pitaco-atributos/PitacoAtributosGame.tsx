...
import { clearAtributosState, loadAtributosState, saveAtributosState } from './storage'
import {
    ...
    <div ref={containerRef} className="relative w-full">
      ...
      >
        ...
        <input
          ...
          aria-expanded={open}
          aria-controls="atributos-listbox"
          aria-invalid={!!error}
          autoComplete="off"
          spellCheck={false}
          inputMode="search"
        ...
        <button
          ...
          onClick={() => {
            if (matches.length > 0) {
              ...
              // Sem match: deixa o engine validar. So fica habilitado
              // quando o usuario tem texto digitado.
              onSubmit({
                id: 'raw',
                nome: value.trim(),
                apelido: value.trim(),
      ...
      <AnimatePresence>
        {open && matches.length > 0 && (
          ...
          >
            {matches.map((a) => (
              <li key={a.id} role="option" aria-selected={value === a.nome || value === a.apelido}>
                <button
                  type="button"
                  onClick={() => {
                    onSubmit(a)
...
function GameOverCard({
  ...
  return (
    <motion.div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="atributos-gameover-title"
      onKeyDown={(e) => {
